...
class CrossAttention(nn.Module):
    ...
    def forward(
        self, hidden_states, encoder_hidden_states=None, attention_mask=None, meta=None
    ):
...
class KNearsetAttention(CrossAttention):
    def forward(
        self, hidden_states, encoder_hidden_states=None, attention_mask=None, meta=None
    ):
        video_length = meta["video_length"]
        k_near_indices = meta["k_near_indices"]  # B X F X K
        batch_size, sequence_length, _ = hidden_states.shape
        ...
        key = repeat(key, "(b f) d c -> b f k d c", f=video_length, k=video_length)

        b, f_views, k_views = k_near_indices.shape
        b_ids = (
            torch.arange(b)[:, None, None].expand(-1, f_views, k_views).to(key.device)
        )
        f_ids = (
            torch.arange(f_views)[None, :, None].expand(b, -1, k_views).to(key.device)
        )

        k_nearest_key = key[b_ids, f_ids, k_near_indices]  # b x f x k x d x c

        key = rearrange(key, "b f k d c -> (b f) (k d) c")

        value = repeat(value, "(b f) d c -> b f k d c", f=video_length, k=video_length)
        k_nearest_value = value[b_ids, f_ids, k_near_indices]  # b x f x k x d x c
        value = rearrange(value, "b f k d c -> (b f) (k d) c")
