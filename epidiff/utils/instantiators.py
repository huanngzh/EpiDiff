...
def instantiate_from_config(config, **kwargs):
    ...
    This method does not use `hydra.utils.instantiate`. Thus, it does not support
    hydra's deep instantiation functionality.
    """
    if not "_target_" in config:
        if config == "__is_unconditional__":
            return None
        raise KeyError("Expected key `_target_` to instantiate.")
    return get_obj_from_str(config.pop("_target_"))(**config, **kwargs)
